...
#include "stdafx.h"
#include "convexpolyintersect.hpp"
namespace pawlin {
	...
	static cv::Point2f cvPoint32f(cv::Point p) {
		cv::Point2f s;
		s.x = (float)p.x;
		s.y = (float)p.y;
		return s;
	...
	bool segementIntersection(cv::Point p0, cv::Point p1, cv::Point p2, cv::Point p3, cv::Point * intersection) {
		cv::Point2f p;
		bool ok = segementIntersection(cvPoint32f(p0), cvPoint32f(p1), cvPoint32f(p2), cvPoint32f(p3), &p);
		if (ok)
			*intersection = cv::Point((int)p.x, (int)p.y);
		return ok;
	}
	...
	float computeArea(const cv::Point * pt, int n) {
		float area0 = 0.f;
		for (int i = 0; i < n; i++) {
			int j = (i + 1) % n;
			area0 += pt[i].x * pt[j].y;
	...
	cv::Point Polygon::getCenter() const {
		cv::Point center;
		center.x = 0;
		center.y = 0;
		auto & pt = *this;
	...
	void Polygon::pointsOrdered() {
		...
		vector<PointAngle> pc(n);
		for (int i = 0; i < n; i++) {
			pc[i].p.x = pt[i].x;
			pc[i].p.y = pt[i].y;
			pc[i].angle = atan2f((float)(pt[i].y - center.y), (float)(pt[i].x - center.x));
		}
		qsort(&pc[0], n, sizeof(PointAngle), comp_point_with_angle);
		for (int i = 0; i < n; i++) {
			pt[i].x = pc[i].p.x;
			pt[i].y = pc[i].p.y;
		}
	...
	bool Polygon::pointIsInPolygon(cv::Point p) const {
		auto & pt = *this;
		int n = (int)size();
		return pointInPolygon(p, &pt[0], n);
	}
	...
	void intersectPolygon(const Polygon & poly0, const Polygon & poly1, Polygon & inter) {
		intersectPolygon(&poly0[0], (int)poly0.size(), &poly1[0], (int)poly1.size(), inter);
	}
	...
	//------------------------------------------------------------------------------------------------------------
	static inline int cross(const cv::Point* a, const cv::Point* b) {
		return a->x * b->y - a->y * b->x;
	}

	static inline cv::Point* vsub(const cv::Point* a, const cv::Point* b, cv::Point* res) {
		res->x = a->x - b->x;
		res->y = a->y - b->y;
		return res;
	}

	static int line_sect(const cv::Point* x0, const cv::Point* x1, const cv::Point* y0, const cv::Point* y1, cv::Point* res) {
		cv::Point dx, dy, d;
		vsub(x1, x0, &dx);
		vsub(y1, y0, &dy);
		vsub(x0, y0, &d);
	...
	static int left_of(const cv::Point* a, const cv::Point* b, const cv::Point* c) {
		cv::Point tmp1, tmp2;
		int x;
		vsub(b, a, &tmp1);
		vsub(c, b, &tmp2);
	...
	static void poly_edge_clip(const Polygon* sub, const  cv::Point* x0, const  cv::Point* x1, int left, Polygon* res) {
		int i, side0, side1;
		cv::Point tmp;
		const cv::Point* v0 = sub->pt() + sub->n() - 1;
		const cv::Point* v1;
		res->clear();
